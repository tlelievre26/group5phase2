...
function uploadBase64Contents(contents) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield (0, s3upload_1.default)("group5phase2packages", `${package_info.name}/${package_info.name}.zip`, zipBinary);
...
exports.uploadBase64Contents = uploadBase64Contents;
function extractFromZip(zip) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const retrieved_files = {}; //Dictionary that will store the filedata of each file we retrieve
        for (const [filename, file] of Object.entries(zip.files)) {
            ...
            if (package_json_match.test(filename)) {
                const fileData = yield file.async('nodebuffer');
                retrieved_files["package.json"] = fileData; //*****ideally should be using retrieved_files[filename]*****
                logger_1.default.debug("Successfully retrieved package.json file from zip");
            }
            else {
                for (const pattern of license_match) {
                    if (pattern.test(filename)) {
                        ...
                        console.log(filename);
                        logger_1.default.debug("Successfully retrieved LICENSE file from zip");
                        break;
                    }
                }
                if (!retrieved_files.hasOwnProperty(filename)) { //If we haven't matched the file to either of the previous 2 patterns
                    for (const pattern of readme_match) {
                        if (pattern.test(filename)) {
                            ...
                            logger_1.default.debug("Successfully retrieved README file from zip");
                            break;
                        }
                    }
                }
        ...
        logger_1.default.debug("Successfully parsed package.json file into JSON object");
        const dir_name = package_json_obj.name;
        for (const [filename, file] of Object.entries(retrieved_files)) {
            yield (0, s3upload_1.default)("group5phase2packages", `${dir_name}/metadata/${filename}`, file);
            logger_1.default.debug(`Successfully uploaded ${filename} to S3 under key ${dir_name}/metadata/${filename}`);
        }
        return package_json_obj;
    });
}
//# sourceMappingURL=unzip_contents.js.map